...
class ManifoldVae(Manifold):
    ...
    def get_datasets(self, batch_size = 100):
        ...
        # MNIST Dataset
        train_dataset = self.dataset(root='./datasets/', train=True, transform=transforms.ToTensor(), download=True)
        test_dataset = self.dataset(root='./datasets/', train=False, transform=transforms.ToTensor(), download=False)
    ...
    def metric_tensor(self, z, nargout=1):
        ...
        def get_metrics(latents):
            ...
            jacobians = functional.jacobian(self.model.decoder, latents, vectorize=True, strategy="forward-mode")
