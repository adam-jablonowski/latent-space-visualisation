...
def select_point_from_input_field(settings, options, prefix):
    ...
    elif options[prefix+'source'] == 'x and y in 2d-reduced dimension (dimesinality reduction reconstruction)':
...
def contour_maps_points(manifold, settings, selected, options):
    ...
    reconstructed = projector.reconstruct([
        *direction_goals, point_view
    ])
    ...
    solver_options = {
        a: options[a] for a in solver_args if options[a] is not None
    }
