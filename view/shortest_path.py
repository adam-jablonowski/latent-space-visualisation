...
class ShortestPaths(Component):
    ...
    def inner_html(self):
        return html.Div([
            ...
            html.Div([
                described_input('x of the beginning in 2d-reduced dimension (float)', 'float', self.ids(0), 4),
                described_input('y of the beginning in 2d-reduced dimension (float)', 'float', self.ids(1), 0),
                described_input('latent value of the beginning (Python`s list of floats) ','text', self.ids(2), ''),
                described_input('input used as the beginning', 'dropdown', self.ids(3), [
                    'latent value',
            ...
            html.Div([
                described_input('x of the end in 2d-reduced dimension (float)', 'float', self.ids(4), 0),
                described_input('y of the end in 2d-reduced dimension (float)', 'float', self.ids(5), 2),
                described_input('latent value of the end (Python`s list of floats) ','text', self.ids(6), ''),
                described_input('input used as the end', 'dropdown', self.ids(7), [
                    'latent value',
                    'x and y in 2d-reduced dimension (dimesinality reduction reconstruction)',
                ]),
                html.Button(
            ...
            html.Div([
                described_input('Number of displayed points', 'float', self.ids(8), 10),
                described_input('Initial number of path nodes (float)', 'float', self.ids(11), 30),
                described_input('Solver`s max number of path nodes', 'number', self.ids(9), 50),
                described_input('Target convergence tol (float)', 'float', self.ids(10), 1),
            ], style=STYLE_BORDER_INNER),
    ...
    def init_callbacks(self, app, manifold, settings, plot):
        ...
        def update_beg(beg_n_clicks, end_n_clicks, click_data, selection_update, *options):
            options = self.options_values(options)
