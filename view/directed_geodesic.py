...
class ExpmapPaths(Component):
    ...
    def inner_html(self):
        return html.Div([
            ...
            html.Div([
                described_input('x of initial direction in 2d-reduced dimension (float)', 'float', self.ids(0), 1),
                described_input('y of initial direction in 2d-reduced dimension (float) ','float', self.ids(1), 0),
            ], style=STYLE_BORDER_INNER),
            html.Div([
                described_input('Time of geodesic curve (float)', 'float', self.ids(2), 1),
                described_input('Number of displayed points', 'number', self.ids(3), 5),
            ], style=STYLE_BORDER_INNER),
            html.Div([
