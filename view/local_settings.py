...
from view.components.html import described_input, twice, axis_input, arrows_input
...
class LocalSettings(Component):
    ...
    def inner_html(self):
        return html.Div([
            ...
            html.Div([
                described_input('Radius for neighbourhood', 'number', self.ids(0), 1),
                described_input('Radius for mean error', 'float', self.ids(1), 0.1),
                *axis_input(
                    'Plot`s color', self.ids(2),
                    'Turn logarithm color on', self.ids(5),
                ),
                *axis_input(
                    'Plot`s z-axis', self.ids(3),
                    'Turn logarithm z-axis on', self.ids(6),
                ),
