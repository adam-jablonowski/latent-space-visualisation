...
class ContourMap(Component):
    ...
    def inner_html(self):
        return html.Div([
            ...
            html.Div([
                described_input('Number of vertical contours', 'number', self.ids(2), 12),
                described_input('Length of initial direction vectors for vertical contours (float)', 'float', self.ids(3), 1),
                described_input('Time of vertical contours curves (float)', 'float', self.ids(0), 3),
                described_input('Number of horizontal contours','number', self.ids(1), 5),
                described_input('Display vertical contours', 'bool', self.ids(12), True),
                described_input('Display horizontal contours', 'bool', self.ids(13), True),
            ], style=STYLE_BORDER_INNER),
            html.Div([
                described_input('Method', 'dropdown', self.ids(4), [
                    'BDF',
                    'LSODA',
                    'Radau',
                    'DOP853',
