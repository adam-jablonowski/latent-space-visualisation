...
from view.components.html import described_input, axis_input, arrows_input
...
class GlobalSettings(Component):
    ...
    def inner_html(self):
        return html.Div([
            ...
            html.Div([
                described_input('Radius for mean error', 'float', self.ids(0), 0),
                *axis_input(
                    'Plot`s color', self.ids(1),
                    'Turn logarithm color on', self.ids(4),
                ),
                *axis_input(
                    'Plot`s z-axis', self.ids(2),
                    'Turn logarithm z-axis on', self.ids(5),
                ),
