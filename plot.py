...
from equations import (all_directions, euclidean_direction, euclidean_neighbors, euclidean_shortest_path,
                       solve_expmap, solve_shortest_path)
...
class PlotFactory:
    ...
    def add_trace(self, fig, trace):
        ...
        color = self.make_axis(trace, self.settings['color'], self.settings['log-color'])
    ...
    def axis_values(self, trace, axis_type):
        if axis_type == 'none':
            ...
            cached = np.concatenate([t['values'][field_name] for t in self.case['sets']], axis=0)
            neighbours = []
    ...
    def markers(self, trace, color, size):
        ...
        if self.settings['color'] == 'measure':
            ...
            marker['cmax'], marker['cmin'] = np.log(max_min) if self.settings['log-color'] else max_min
    ...
    def arrows(self, trace):
        ...
        elif self.settings['arrows'] == '-> reconstruction + decoder + encoder + reduction':
            reduced = self.reducer.reduce(trace['values']['latent'])
            latent = self.reducer.reconstruct(reduced)
